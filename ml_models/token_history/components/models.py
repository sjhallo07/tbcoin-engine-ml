...
"""Torch-backed model with graceful fallback when torch is unavailable.

On environments where PyTorch wheels are not available (e.g., Python 3.14),
we expose a minimal dummy model so the pipeline and API can run without error.
"""
...
if TORCH_AVAILABLE:
    ...
    class TokenMovementTransformer:
        """Torch-less dummy model used when PyTorch is unavailable."""
        def __init__(self, config):
            self.config = config

        def __call__(self, x, attention_mask=None):
            return {"amount": 0.0, "address": 0.0, "time": 0.0}

        # For API consistency
        def forward(self, x, attention_mask=None):
            return self.__call__(x, attention_mask)
